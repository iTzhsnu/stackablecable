...
public class AP_Tool_Base extends Item {
    private final Set<Block> blocks;
    private final int mineLv;

    public AP_Tool_Base(ToolType toolType, int mineLv, Set<Block> blocks, Properties properties) {
        super(properties.addToolType(toolType, mineLv).defaultDurability(1000000));
        this.blocks = blocks;
        this.mineLv = mineLv;
    }
    ...
    public float getDestroySpeed(ItemStack stack, BlockState state) {
        if (stack.getTag() != null && stack.getTag().contains("Mining_Speed")) {
            if (getToolTypes(stack).stream().anyMatch(state::isToolEffective)) {
                return stack.getTag().getFloat("Mining_Speed");
            }
            return this.blocks.contains(state.getBlock()) ? stack.getTag().getFloat("Mining_Speed") : 1.0F;
        }
        return 0.0F;
    }
    ...
    public boolean hurtEnemy(ItemStack stack, LivingEntity target, LivingEntity attacker) {
        stack.hurtAndBreak(2, attacker, (p_220039_0_) -> p_220039_0_.broadcastBreakEvent(EquipmentSlotType.MAINHAND));
        return true;
    }
    ...
    public boolean mineBlock(ItemStack stack, World world, BlockState state, BlockPos pos, LivingEntity entity) {
        if (!world.isClientSide && state.getDestroySpeed(world, pos) != 0.0F) {
            stack.hurtAndBreak(1, entity, (p_220038_0_) -> p_220038_0_.broadcastBreakEvent(EquipmentSlotType.MAINHAND));
        }
        return true;
    }
    ...
    public void appendHoverText(ItemStack stack, @Nullable World world, List<ITextComponent> text, ITooltipFlag flag) {
        if (stack.getTag() != null) {
            ...
            text.add(new TranslationTextComponent("text.scable.mining_level").append(String.valueOf(getMineLv())));
